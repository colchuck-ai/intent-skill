...
Make the edit with propagation in mind: if requirement links or the table row change, update the engineering root in the same story. Prefer one coherent narrative over a scattered fix.

### AI-assisted review

Check consistency across component, table, and product requirements. The **engineering README** row should still be where full requirement links live; the spec should not sprout a parallel mapping block. Emit **CRs** for edits not yet traced; **ADRs** for new decisions.

### Independent review

Human pass for ownership and blast radius. **CRs** for meaningful late edits; **ADRs** if decisions surface at the gate.
